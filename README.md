...
* Run the interactive REPL with `python adarsh_lang_compiler.py --repl` and finish a snippet with a blank line.
...
Legacy imports (`import adarsh_lang_compiler`) continue to work via a thin compatibility module at the repository root.
...
* Package and ship the `adarsh_lang` directory along with `adarsh_lang_compiler.py` for compatibility.
